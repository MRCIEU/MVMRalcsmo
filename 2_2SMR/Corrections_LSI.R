...
SmoInit<-read.table("/user/work/bu19525/BristolPhD/MVMR_follow_up/Data/Lifetime_smoking.txt",sep=" ",header=TRUE,row.names = NULL)
head(SmoInit)
...
mr_results<-mr(H_data, method_list=c("mr_wald_ratio", "mr_egger_regression", "mr_ivw", "mr_two_sample_ml","mr_weighted_median","mr_weighted_mode"))
mr_results$id.exposure<-"Smoking"
mr_results
mr_smo<-mr_results
FullResults<-merge(FullResults,mr_results)
Presso<-run_mr_presso(H_data)
...
#MR_merge<-rbind(MR_merge,mr_smo)

setwd("/mnt/storage/scratch/bu19525/BristolPhD/MVMR_follow_up/Output/Corrections/")
write.csv(MR_merge,"Full2SMR_LSI.csv")
write.table(MR_merge,"Full2SMR_LSI.txt")
